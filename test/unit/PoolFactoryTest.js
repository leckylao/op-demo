...
describe("PoolFactory", function() {
    ...
    it("Should be able to createFund", async function() {
        ...
        await expect(poolFactory.createFund(
            false, manager.address, 'Barren Wuffet', 'Test Fund', "DHTF", new ethers.BigNumber.from('6000'), [sethKey, sbtcKey]
        ))
    ...
    it('should be able to deposit', async function() {
        ...
        await poolLogicProxy.deposit(10e18.toString())

        let event = await depositEvent;

        expect(event.fundAddress).to.equal(poolLogicProxy.address);
        expect(event.investor).to.equal(logicOwner.address);
        expect(event.valueDeposited).to.equal(10e18.toString());
        expect(event.fundTokensReceived).to.equal(10e18.toString());
        expect(event.totalInvestorFundTokens).to.equal(10e18.toString());
        expect(event.fundValue).to.equal(12e18.toString());
        expect(event.totalSupply).to.equal(10e18.toString());
    });
    ...
    it('should be able to deposit again', async function() {
        ...
        expect(event.investor).to.equal(logicOwner.address);
        expect(event.valueDeposited).to.equal(10e18.toString());
        expect(event.fundTokensReceived).to.equal(50e18.toString());
        expect(event.totalInvestorFundTokens).to.equal(60e18.toString());
        expect(event.fundValue).to.equal(12e18.toString());
        expect(event.totalSupply).to.equal(60e18.toString());
    });

    it('should be able to exchange', async function() {
        await expect(poolLogicProxy.exchange(susdKey, 100e18.toString(), sethKey))
        ...
        let exchangeEvent = new Promise((resolve, reject) => {
            poolLogicManagerProxy.on('Exchange', (
                poolLogicAddress,
                manager,
                sourceKey,
                sourceAmount,
        ...
        await poolLogicManagerProxy.exchange(susdKey, 100e18.toString(), sethKey);
    ...
    it('should be able to manage assets', async function() {
        ...
        await expect(poolManagerLogicUser1Proxy.removeFromSupportedAssets(poolLogicProxy.address, slinkKey))
            .to.be.revertedWith('only manager, trader or DAO');

        await expect(poolManagerLogicManagerProxy.removeFromSupportedAssets(poolLogicProxy.address, susdKey))
        ...
        await expect(poolManagerLogicManagerProxy.removeFromSupportedAssets(poolLogicProxy.address, slinkKey))
            .to.be.revertedWith("revert cannot remove non-empty asset");

        // Can remove asset
        await mock.givenMethodReturnUint(balanceOfABI, 0)
        await poolManagerLogicManagerProxy.removeFromSupportedAssets(poolLogicProxy.address, slinkKey)
